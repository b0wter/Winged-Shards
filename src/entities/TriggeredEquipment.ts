...
export abstract class TriggeredEquipment extends Equipment
{
     ...
     */
    public trigger(scene: GameplayScene, colliderFunc: AddProjectileFunc, equipmentPosition: EquipmentPositionCallback, angle: EquipmentAngleCallback, time: number, ownerId: string) : number
    {
        const passed = time - this.lastUsedAt
        if(passed > this.cooldown * this.cooldownModifier) {
            this.internalTrigger(scene, colliderFunc, equipmentPosition, angle, time, ownerId)
    ...
    protected abstract internalTrigger(scene: GameplayScene, colliderFunc: AddProjectileFunc, equipmentPosition: EquipmentPositionCallback, angle: EquipmentAngleCallback, time, ownerId: string)
...
{
    public abstract instantiate(team: Teams)
} 